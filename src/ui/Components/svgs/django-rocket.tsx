...
/**
 * DjangoRocket - A stylized rocket illustration representing Django.
 *
 * Renders a multi-layered SVG rocket with smoke, exhaust flame, and a
 * Django-themed color scheme using CSS custom properties for theming.
 */
export function DjangoRocket(props: SVGProps<SVGSVGElement> & { title?: string }) {
	return (
		<svg
			className="figure"
			viewBox="0 0 508 268"
			aria-label="Django Rocket Illustration"
			{...props}>
			<title>{props.title ?? "Django rocket illustration"}</title>
			<desc>A stylized rocket illustration representing Django.</desc>
			<path
			...
			<path
				fill="var(--error)"
				d="M239 152h30v8h-30z"
			/>
			<path
				className="exhaust__line"
				fill="var(--text-tertiary)"
				d="M250 172h7v90h-7z"
			/>
			<path
